...
filename = "data/query.titles.tsv"
...
df['rank'] = df.groupby('topic')['document_score'].rank(ascending=False).astype(int)
df['run_tag'] = np.array(['bm25'] * input_size)

columns = ['topic', 'Q0', 'document_name', 'rank', 'document_score', 'run_tag']
bm25_ranking = df[columns]
bm25_ranking.to_csv(path_or_buf='results/bm25_ranking.txt', sep=' ', index=False, header=False)
